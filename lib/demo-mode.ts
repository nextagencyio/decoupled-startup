...
import landingPagesData from '@/data/mock/about.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetLandingPages') || query.includes('nodeLandingPages')) {
      return loadMockData('about.json')
    }
